$(document).ready(function () {
	VnExpress.init();
	if (window.location.href.indexOf("vn_source=") != -1) setTimeout(function () {
...
var homePage = [],
	...
	VnExpress = {
		...
		interactionsParser: function (action) {
			if (action === "cmt.widget"){
			}
			else if (action === "comment") {
			}
			else if (action === "vote.widget" || action === "quote.widget" || action === "like.widget") {
		...
		animateLoading: function (element, action) {
			...
			if (action == "show") {
				var loadHtml = "";
				for (i = 1; i <= 4; ++i) loadHtml += '                <article class="list_item clearfix box_loading_2">                    <div class="thumb_100">                        <div class="box-line-thumb"></div>                    </div>                    <div class="content_item">                        <h2 class="title_item">                            <div class="box-line-title"></div>                        </h2>                        <div class="meta">                            <div class="box-line-author"></div>                        </div>                        <div class="description">                            <div class="box-line-desc-first"></div>                            <div class="box-line-desc-second"></div>                        </div>                    </div>                </article>                ';
		...
		history: function () {
			...
			var gocnhin_reader_list = $.cookie("gocnhin_reader_list"),
				gocnhin_reader_arr = [],
				gocnhin_reader_id = $("meta[name=tt_article_id]").attr("content");
			gocnhin_reader_id = parseInt(gocnhin_reader_id);
			gocnhin_reader_arr.push(gocnhin_reader_id);
			if (typeof gocnhin_reader_list != "undefined") $.each($.parseJSON(gocnhin_reader_list), function (k, v) {
				if (gocnhin_reader_arr.length < 10 && $.inArray(v, gocnhin_reader_arr) < 0) gocnhin_reader_arr.push(v)
			});
			$.cookie.json = true;
			$.cookie("gocnhin_reader_list", gocnhin_reader_arr, {
				expires: 30,
				path: "/"
			})
		},
